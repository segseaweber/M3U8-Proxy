...
export async function proxyM3U8(url: string, headers: any, res: http.ServerResponse) {
    try {
        const response = await axios(url, {
            headers: {
                ...headers,
                'accept-encoding': 'identity' // Avoid gzip
            },
            responseType: 'text',
            timeout: 10000
        });
        ...
        res.writeHead(500);
        res.end('Failed to fetch M3U8: ' + error.message);
    }
}
...
export async function proxyTs(url: string, headers: any, req, res: http.ServerResponse) {
    try {
        ...
        const options = {
            ...
            method: req.method,
            headers: {
                ...headers, // Your custom headers including referer
                host: uri.hostname, // Important: set correct host header
                'accept-encoding': 'identity' // Prevent gzip to avoid issues with binary data
            }
        };
        ...
        const proxyReq = proxyRequest(options, (proxyRes) => {
            ...
            ['content-encoding', 'transfer-encoding', 'connection'].forEach(header => {
        ...
        if (!res.headersSent) {
            res.writeHead(500);
            res.end('Setup error: ' + error.message);
        }
    }
}
