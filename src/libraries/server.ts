...
export default function server() {
    ...
    }).listen(Number(port), host, function () {
        ...
        unlimitedPattern
            ...
            .forEach(function (unlimitedHost, i) {
                ...
                if (unlimitedHost.length > 1 && startsWithSlash && endsWithSlash) {
                    ...
                    new RegExp(unlimitedHost); // Validate regex
                    unlimitedPatternParts.push(unlimitedHost);
                } else {
                    unlimitedPatternParts.push(unlimitedHost.replace(/[$()*+.?[\\\]^{|}]/g, "\\$&"));
                }
            });
...
export async function proxyM3U8(url: string, headers: any, res: http.ServerResponse) {
    try {
        ...
        // Process EXT-X-KEY tags
        m3u8Content = m3u8Content.replace(/#EXT-X-KEY:(.*?)URI="(.*?)"(.*)/gi, (match, p1, uri, p3) => {
            const proxyUrl = `${web_server_url}/ts-proxy?url=${encodeURIComponent(uri)}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
            return `#EXT-X-KEY:${p1}URI="${proxyUrl}"${p3}`;
        });

        // Process all non-comment lines (TS segments and playlists)
        ...
        const processedLines = lines.map(line => {
            ...
            try {
                // Check if this is a TS file or another m3u8
                const segmentUrl = new URL(line, url).href;
                if (segmentUrl.includes('.m3u8')) {
                    // It's another playlist, use m3u8-proxy
                    return `${web_server_url}/m3u8-proxy?url=${encodeURIComponent(segmentUrl)}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
                } else {
                    // It's a TS file, use ts-proxy
                    return `${web_server_url}/ts-proxy?url=${encodeURIComponent(segmentUrl)}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
                }
            } catch (error) {
                return line;
            }
        ...
        res.end(processedLines.join('\n'));

    } catch (error) {
        console.error('M3U8 proxy error:', error);
        res.writeHead(500);
        res.end('Failed to fetch M3U8: ' + (error as Error).message);
    }
}
...
export async function proxyTs(url: string, headers: any, req, res: http.ServerResponse) {
    try {
        // Validate that this is actually a TS file request
        if (!url.includes('.ts') && !url.includes('.m4s') && !url.includes('.m4f')) {
            console.log('Not a TS file, returning 400:', url);
            res.writeHead(400);
            res.end('Invalid TS file request');
            return;
        }
        ...
        const options = {
            ...
            method: 'GET', // Always use GET for TS files
            headers: {
                ...cleanHeaders,
                'host': uri.hostname,
                'accept': '*/*',
                'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
                'accept-encoding': 'identity' // Prevent compression for binary data
            },
            timeout: 30000
        };

        const proxyRequest = isHttps ? https.request : http.request;
        ...
        proxyReq.on('error', (error) => {
            console.error('Proxy TS error:', error.message, 'for URL:', url);
            if (!res.headersSent) {
                res.writeHead(500);
        ...
        proxyReq.on('timeout', () => {
            console.error('Proxy TS timeout for URL:', url);
            proxyReq.destroy();
            if (!res.headersSent) {
                res.writeHead(504);
        ...
        // For TS files, we don't need to pipe the original request body
        // since it's a GET request. Just end the proxy request.
        proxyReq.end();

    } catch (error) {
        console.error('Proxy TS setup error:', error);
        if (!res.headersSent) {
            res.writeHead(500);
            res.end('Setup error: ' + (error as Error).message);
        }
    }
}
